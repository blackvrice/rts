...
- Routed minimap fog cells, entity blips, and camera viewport through the same fitted map rectangle. Blips and the viewport are clamped to the visible map bounds so off-map positions do not draw into the panel padding.
- Changed minimap click handling to use the fitted map rectangle too, so clicks in letterbox padding do not translate to world orders or camera jumps.
- Verification: built `RTS` with `C:\Program Files\JetBrains\CLion 2026.1.2\bin\cmake\win\x64\bin\cmake.exe`, ran `cmake-build-debug\rts_headless_smoke.exe`, and launched `cmake-build-debug\RTS.exe` for a 5-second smoke run.
