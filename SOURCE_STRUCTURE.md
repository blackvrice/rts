...
- `include/core/sim` holds the simulation timing/determinism foundation: `SimClock.hpp` (fixed logic tick rate constants + `TickCount`) and `Fixed.hpp` (16.16 fixed-point `Fixed`/`FixedVec2` + grid/world conversions, compile-time tested via `src/core/sim/Fixed.cpp`). The logic thread steps at `SimClock`'s fixed delta; `GameWorld::currentTick()` is the monotonic tick index. Fixed-point types are the determinism foundation and are not yet wired into the live float-based simulation.
