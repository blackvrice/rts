...
- After any code change, build the project and run the built executable/game before committing.
  - **IMPORTANT**: When building or configuring with CMake, ALWAYS use the executable located at `C:\Program Files\JetBrains\CLion 2026.1.2\bin\cmake\win\x64\bin\cmake.exe` (or prepend `C:\Program Files\JetBrains\CLion 2026.1.2\bin\cmake\win\x64\bin` to your `PATH` temporarily).
...
- Keep `SOURCE_STRUCTURE.md` updated when adding, moving, renaming, or removing source modules, folders, entry points, major runtime systems, or build/runtime ownership.
- Source-structure notes should explain the purpose of important folders, where headers and implementations live, which files are entry points, and which build/run commands are relevant.
